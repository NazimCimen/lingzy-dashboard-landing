...
export function IPhoneMockup({ src, alt, className = "", priority = false }: IPhoneMockupProps) {
  return (
    <div className={`relative ${priority ? "hero-phone-wrap" : ""} ${className}`}>
      {priority && <div className="hero-phone-glow" aria-hidden />}

      {/* iPhone Frame */}
      <div className={`relative mx-auto w-[280px] h-[572px] rounded-[3rem] p-[10px] shadow-2xl ${priority ? "hero-phone-frame" : "bg-foreground"}`}>
        {/* Inner bezel */}
        <div className="relative w-full h-full bg-foreground rounded-[2.5rem] overflow-hidden">
          ...
          <div className="w-full h-full rounded-[2.4rem] overflow-hidden bg-card">
            <img src={src} alt={alt} className={`w-full h-full object-cover object-top ${priority ? "hero-screen-pan" : ""}`} />
          </div>
        </div>
      </div>
    </div>
